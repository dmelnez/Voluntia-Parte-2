					...
					<div class="mx-auto w-full bg-white p-6 shadow rounded-xl">
						<div class="mx-auto w-full max-w-[1000px] bg-white">
							<form class="needs-validation" novalidate
								action="/VistaVoluntia/plan" method="post">
								<input type="hidden" name="accion" value="modificar" /> <input
									type="hidden" id="idPlan" name="idPlan"
									value="<%=request.getParameter("idPlan") != null ? request.getParameter("idPlan") : ""%>" />

								<div class="-mx-3 flex flex-wrap">
									<div class="w-full px-3 sm:w-1/2">
										<div class="mb-5">
											<label for="tipoPlan"
												class="mb-3 block text-base font-medium text-[#07074D]">Tipo
												Plan</label> <input type="text"
												value="<%=request.getParameter("tipoPlan")%>"
												name="tipoPlan" id="tipoPlan" placeholder="Tipo Plan"
									...
									<div class="w-full px-3 sm:w-1/2">
										<div class="mb-5">
											<label for="precioPlan"
												class="mb-3 block text-base font-medium text-[#07074D]">Precio
												Plan</label> <input type="number"
												value="<%=request.getParameter("precioPlan")%>"
												name="precioPlan" id="precioPlan" placeholder="Precio"
												class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
										</div>
									</div>
								</div>
									...
									<div class="w-full px-3 sm:w-1/2">
										<div class="mb-5">
											<label for="tiempoPlan"
												class="mb-3 block text-base font-medium text-[#07074D]">Duración
												Plan</label> <input type="text"
												value="<%=request.getParameter("tiempoPlan")%>"
												name="tiempoPlan" id="tiempoPlan"
												class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
												placeholder="Duración Plan" />
										</div>
									</div>
									<div class="w-full px-3 sm:w-1/2">
										<div class="mb-5">
											<label for="descripcionPlan"
												class="mb-3 block text-base font-medium text-[#07074D]">Descripción
												Plan</label> <input type="text"
												value="<%=request.getParameter("descripcionPlan")%>"
												name="descripcionPlan" id="descripcionPlan"
												class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
									...
									<div class="w-full px-3 sm:w-1/2">
										<div class="mb-5">
											<label for="numeroUsuariosPlan"
												class="mb-3 block text-base font-medium text-[#07074D]">Número
												Usuarios</label> <input type="number"
												value="<%=request.getParameter("numeroUsuariosPlan")%>"
												name="numeroUsuariosPlan" id="numeroUsuariosPlan"
												placeholder="Número Usuarios"
												class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
										</div>
									</div>
								</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (mensaje) {
        ...
        // Se elimina la notificación después de 3 segundos y redirige si es éxito
        setTimeout(() => {
            container.remove();
            if (status === 'success') {
                window.location.href = '/VistaVoluntia/RolAdministradorEmpresa/EmpresaAdminTablaPlanes.html';
            }
        }, 3000);
    }
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener("DOMContentLoaded", function () {
	    var planData = sessionStorage.getItem('planModificar');

	    if (planData) {
	        var plan = JSON.parse(planData);
	        document.getElementById("idPlan").value = plan.idPlan;

	        // Asignar valores a los campos del formulario
	        document.getElementById("tipoPlan").value = plan.tipoPlan;
	        document.getElementById("precioPlan").value = plan.precioPlan;
	        document.getElementById("tiempoPlan").value = plan.tiempoPlan;
	        document.getElementById("descripcionPlan").value = plan.descripcionPlan;
	        document.getElementById("numeroUsuariosPlan").value = plan.numeroUsuariosPlan;
	    }
	});
