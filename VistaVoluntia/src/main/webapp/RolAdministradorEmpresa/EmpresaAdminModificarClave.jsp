					...
					<div class="mx-auto w-full bg-white p-6 shadow rounded-xl">
						<div class="mx-auto w-full max-w-[1000px] bg-white">
							<form class="needs-validation" novalidate
								action="/VistaVoluntia/clave" method="post"
								class="grid grid-cols-1 lg:grid-cols-2 gap-6">
								<input type="hidden" name="accion" value="modificar" /> <input
									type="hidden" id="idClave" name="idClave"
									value="<%=request.getParameter("idClave") != null ? request.getParameter("idClave") : ""%>" />
									...
									<div class="w-full px-3 sm:w-1/2">
										<div class="mb-5">
											<label for="nombreClave"
												...
												Nombre Clave </label> <input type="text"
												value="<%=request.getParameter("nombreClave")%>"
												name="nombreClave" id="nombreClave"
									...
									<div class="w-full px-3 sm:w-1/2">
										<div class="mb-5">
											<label for="descripcionClave"
												class="mb-3 block text-base font-medium text-[#07074D]">
												Descripcion Clave </label> <input type="text"
												value="<%=request.getParameter("descripcionClave")%>"
												name="descripcionClave" id="descripcionClave"
												placeholder="Descripcion Clave"
												class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
										</div>
									</div>
								</div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener("DOMContentLoaded", function () {
	    var claveData = sessionStorage.getItem('claveModificar');

	    if (claveData) {
	        var clave = JSON.parse(claveData);

	        document.getElementById("idClave").value = clave.idClave;	        
	        document.getElementById("nombreClave").value = clave.nombreClave;
	        document.getElementById("descripcionClave").value = clave.descripcionClave;
	    }
	});
