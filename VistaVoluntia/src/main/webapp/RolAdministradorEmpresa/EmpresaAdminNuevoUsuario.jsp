		...
		<main>
			<div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
								...
								<div class="w-full px-3 sm:w-1/2">
									<div class="mb-5">
										...
										<p class="text-red-500 text-sm mt-2 hidden"
											id="apellidosError">Los apellidos son obligatorios</p>
									</div>
								</div>
							</div>
              ...
              hover:file:bg-violet-100" />
							...
							<div class="mb-5">
								<label for="idOrganizacion"
									class="block text-base font-medium text-[#07074D]">Organización</label>
								<select id="idOrganizacion" name="idOrganizacion"
									class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6">
									<option value="">Usar organización de la sesión</option>
								</select>
							</div>
...
document.addEventListener('DOMContentLoaded', function(){
  fetch('http://localhost:8080/VistaVoluntia/organizacion?accion=listaOrganizaciones')
    .then(res => res.json())
    .then(orgs => {
      const sel = document.getElementById('idOrganizacion');
      orgs.forEach(o => {
        const opt = document.createElement('option');
        opt.value = o.idOrganizacion;
        opt.textContent = o.nombreOrganizacion;
        sel.appendChild(opt);
      });
    })
	...
	<script>
          ...
          `;
                            ...
                            setTimeout(() => {
                                container.remove();
                                window.location.href = "/VistaVoluntia/RolAdministradorEmpresa/EmpresaAdminNuevoUsuario.jsp";
                            }, 3000);
                        }
                    });
                </script>
